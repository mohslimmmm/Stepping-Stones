...
const Dashboard = () => {
  // Mock KPIs
  const kpis = [
    { label: 'Daily Clinic Flow', value: '14', change: '+2', icon: <Users className="text-blush" size={24} /> },
    { label: 'Product Waitlist', value: '38', change: '+12%', icon: <PackageSearch className="text-blush" size={24} /> },
    { label: 'Follow-ups Due', value: '9', change: '-3', icon: <CalendarCheck className="text-blush" size={24} /> },
    { label: 'Monthly Revenue', value: '£42.5k', change: '+8%', icon: <TrendingUp className="text-blush" size={24} /> }
  ];

  return (
    <div className="fade-in max-w-6xl mx-auto">
      <div className="mb-8">
        <h2 className="text-3xl font-header text-slate-dark mb-2">Overview</h2>
        <p className="text-slate opacity-80 font-body">Current clinic status and operational metrics.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        {kpis.map((kpi, index) => (
          <div key={index} className="bg-white p-6 border border-slate-grey/20 shadow-sm hover-lift">
            ...
            <h3 className="text-3xl font-header text-slate-dark mb-1">{kpi.value}</h3>
            <p className="text-xs uppercase tracking-widest font-bold text-slate opacity-60">{kpi.label}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-slate-grey/20 shadow-sm p-6">
          <h3 className="font-header text-xl text-slate-dark mb-6 border-b border-slate-grey/10 pb-4">Today's Schedule</h3>
          <div className="space-y-4">
            {[
              ...
              <div key={i} className="flex items-center justify-between p-4 border border-slate-grey/10 hover:bg-slate-dark/5 transition-colors">
                <div className="flex gap-6 items-center">
                  <span className="font-header text-lg text-slate-dark w-16">{appt.time}</span>
                  <div>
                    <strong className="block text-slate-dark font-body">{appt.patient}</strong>
                    <span className="text-xs uppercase tracking-widest text-slate opacity-70">{appt.procedure}</span>
                  </div>
                </div>
                <div>
                  <span className={`text-xs px-3 py-1 font-body font-bold border ${appt.status === 'In Progress' ? 'border-blush text-blush' : 'border-slate-grey/30 text-slate opacity-70'}`}>
                    {appt.status}
                  </span>
                </div>
              </div>
        ...
        <div className="bg-slate-dark text-ivory p-6 shadow-sm">
          <h3 className="font-header text-xl mb-6 tracking-wide text-blush">Quick Actions</h3>
