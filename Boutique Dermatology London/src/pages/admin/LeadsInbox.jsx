...
const LeadsInbox = () => {
  ...
  const getUrgencyIcon = (urgency) => {
    switch(urgency) {
      ...
      default: return <CheckCircle size={16} className="text-slate" />;
    }
  };
  ...
  return (
    <div className="fade-in max-w-6xl mx-auto h-full flex flex-col">
      <div className="mb-8 border-b border-slate-grey/20 pb-4">
        <h2 className="text-3xl font-header text-slate-dark mb-2">Leads Inbox</h2>
        <p className="text-slate opacity-80 font-body">Manage and triage patient consultation requests.</p>
      </div>
        ...
        <div className="md:col-span-1 bg-white border border-slate-grey/20 flex flex-col shadow-sm">
          <div className="p-4 border-b border-slate-grey/10">
            <div className="relative mb-4">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-slate opacity-50" size={16} />
            ...
            <div className="flex gap-2 text-xs uppercase tracking-widest font-bold">
              <button 
                className={`flex-1 py-1 text-center border transition-colors ${filter === 'all' ? 'border-slate-dark text-slate-dark' : 'border-transparent text-slate hover:border-slate-grey/20'}`}
                onClick={() => setFilter('all')}
              >
                All
              </button>
              <button 
                className={`flex-1 py-1 text-center border transition-colors ${filter === 'unread' ? 'border-slate-dark text-slate-dark' : 'border-transparent text-slate hover:border-slate-grey/20'}`}
          ...
          <div className="overflow-y-auto flex-grow p-2 space-y-2">
            {leads.map((lead) => (
              <div key={lead.id} className={`p-4 border border-slate-grey/10 cursor-pointer hover:bg-slate-grey/5 transition-colors ${lead.status === 'Unread' ? 'bg-white border-l-4 border-l-blush' : 'bg-slate-grey/5'}`}>
                <div className="flex justify-between items-start mb-1">
                  <h4 className={`font-body text-sm ${lead.status === 'Unread' ? 'font-bold text-slate-dark' : 'text-slate'}`}>{lead.name}</h4>
                  <span className="text-xs text-slate opacity-60">{lead.date}</span>
                </div>
                <div className="flex justify-between items-center mt-3">
                  <span className="text-xs uppercase tracking-widest text-slate opacity-80">{lead.service}</span>
        ...
        <div className="md:col-span-2 bg-white border border-slate-grey/20 p-8 shadow-sm flex flex-col">
          <div className="border-b border-slate-grey/10 pb-6 mb-6">
            <div className="flex justify-between items-start mb-4">
              <div>
                <h3 className="text-2xl font-header text-slate-dark mb-1">Eleanor Vance</h3>
                <span className="text-sm font-body text-slate opacity-80 flex items-center gap-2"><Mail size={14} /> eleanor.vance@example.com</span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-6 bg-slate-grey/5 p-4 border border-slate-grey/10">
              <div>
                <span className="block text-xs uppercase tracking-widest text-slate opacity-60 font-bold mb-1">Interested In</span>
                <span className="font-body text-slate-dark text-sm">Bespoke Laser Resurfacing</span>
              </div>
              <div>
                <span className="block text-xs uppercase tracking-widest text-slate opacity-60 font-bold mb-1">Phone Number</span>
                <span className="font-body text-slate-dark text-sm">+44 7700 900077</span>
              </div>
            </div>
          </div>

          <div className="flex-grow">
            <h4 className="text-xs uppercase tracking-widest text-slate opacity-60 font-bold mb-3">Message</h4>
            <div className="font-body text-slate leading-relaxed text-sm bg-ivory p-6 border border-slate-grey/10">
